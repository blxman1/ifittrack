package com.taha.fittrack.fragment;
...
import com.taha.fittrack.activity.GoalsActivity;
import com.taha.fittrack.R;
import com.taha.fittrack.activity.AddWorkoutActivity;
import com.taha.fittrack.activity.DiaryActivity;
import com.taha.fittrack.activity.FoodListActivity;
...
public class HomeFragment extends Fragment
{
    ...
    private String strUserCurrentWeight, strUserGoalWeight, strUserWeeklyGoalWeight, strUserStartWeight, strUserBYear, strUserAge, strUserHeight,
            ...
            strUserTDEE, strUserCaloriesRemaining, strUserBmi, strUserHealthStatus, strUserWaterGlasses, strUserGoalType="lose";
                ...
                if(dataSnapshot.exists())
                {
                    ...
                    if(weightGoalStatus.equals("true") && weightAchievementStatus.equals("false"))
                    {
                        if(!strUserCurrentWeight.isEmpty() && !strUserGoalWeight.isEmpty())
                        {
                            ...
                            if(strUserGoalType.equals("gain"))
                            {
                                /* For weight gain: current weight >= goal weight */
                                goalAchieved = Double.parseDouble(strUserCurrentWeight) >= Double.parseDouble(strUserGoalWeight);
                            }
                            else
                            {
                                /* For weight loss: current weight <= goal weight */
                                goalAchieved = Double.parseDouble(strUserCurrentWeight) <= Double.parseDouble(strUserGoalWeight);
                            }
    ...
    private void PopupAchievementDialog(String points)
    {
        ...
        String goalTypeText = strUserGoalType.equals("gain") ? "weight gain" : "weight loss";
        dialogDescription.setText("Congratulations, You have achieved your " + goalTypeText + " goal.");
        ...
        userDatabaseReference.addValueEventListener(new ValueEventListener()
        {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
            {
                if(dataSnapshot.exists())
                {
                    ...
                    if(dataSnapshot.hasChild("usergoaltype"))
                    {
                        strUserGoalType = dataSnapshot.child("usergoaltype").getValue().toString();
                    }
    ...
    private void SetUserDataToHomePage()
    {
                ...
                weightAchievementStatus.equals("false"))
        {
            ...
            if(strUserGoalType.equals("gain"))
            {
                /* For weight gain: add calories to TDEE */
                strUserTDEE = String.valueOf(Integer.parseInt(strUserTDEE) + Integer.parseInt(cutCaloriesPerDay));

                /* Calculate gain weight goal and gained weight */
                String gainWeightGoal = String.format(Locale.US,"%.1f",(Double.parseDouble(strUserGoalWeight) - Double.parseDouble(strUserStartWeight)));
                String gainedWeight = String.format(Locale.US,"%.1f",(Double.parseDouble(strUserCurrentWeight) - Double.parseDouble(strUserStartWeight)));

                String weightGainProgressValue = String.format(Locale.US,"%.0f",((Double.parseDouble(gainedWeight) * 100) / Double.parseDouble(gainWeightGoal)));
                weightCardGoalProgress.setProgress(Integer.parseInt(weightGainProgressValue));
            }
            else
            {
                /* For weight loss: subtract calories from TDEE */
                strUserTDEE = String.valueOf(Integer.parseInt(strUserTDEE) - Integer.parseInt(cutCaloriesPerDay));

                /* Calculate loss weight goal and lost weight */
        ...
        {
            weightCardGoalCompleteDescription.setText("Set a weight goal and earn points.");
        }
